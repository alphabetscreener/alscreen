...
async function debugBatchStrict() {
    const titles = [
    ...
    console.log(`Fetching batch of ${titles.length} failing titles with STRICT prompt...`);
    ...
    const prompt = `You are an image search assistant. 
      ...
      For EACH title:
      1. Use the Google Search tool to find a valid image from 'themoviedb.org' (image.tmdb.org) or 'wikimedia.org'.
      2. If found, return that specific URL.
      3. If NOT found on those sites, return null. DO NOT use random sites like rogerebert.com, ebay, or pinterest.
      
      Return a valid JSON object string where the keys are the exact titles provided and the values are the image URLs.
      Example:
      \`\`\`json
      { "Title A": "https://image.tmdb.org/...", "Title B": "https://upload.wikimedia.org/..." }
      \`\`\`
      `;
