...
const sysMsg = `You are a media database assistant. SEARCH THE WEB for the title's plot, parental guide, and ratings.
               ...
               IMPORTANT: Search specifically for "Rotten Tomatoes" score, "Rotten Tomatoes URL", and "IMDb ID" (tt code).
...
const prompt = `Analyze: "The Matrix". 
                   
                   STEP 1: DISAMBIGUATION CHECK
                   - Does this title refer to a FRANCHISE, SERIES, or multiple DISTINCT movies/shows (e.g. "Lilo & Stitch" has a movie, a show, and sequels)?
...
async function test() {
    ...
    try {
        ...
        const atpMatch = text.match(/(?:ATP|TPS)(?:\*\*|):?\s*(\d+(?:\.\d+)?)/i);
        const rationaleMatch = text.match(/(?:Rationale)(?:\*\*|):?\s*([\s\S]+)/i);
        const rtUrlMatch = text.match(/(?:Rotten Tomatoes URL)(?:\*\*|):?\s*(https?:\/\/[^\s]+)/i);

        console.log("\nParsing Results:");
        console.log("Title Match:", titleMatch ? "YES" : "NO", titleMatch ? titleMatch[1] : "");
        console.log("ATP Match:", atpMatch ? "YES" : "NO", atpMatch ? atpMatch[1] : "");
        console.log("Rationale Match:", rationaleMatch ? "YES" : "NO");
        console.log("RT URL Match:", rtUrlMatch ? "YES" : "NO", rtUrlMatch ? rtUrlMatch[1] : "");

        if (titleMatch && atpMatch && rationaleMatch) {
            console.log("SUCCESS: Analysis parsed correctly.");
        } else {
            console.log("FAILURE: Analysis parsing failed.");
        }
